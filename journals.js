...
async function run() {
    ...
    // получаем обложки
    const images = await page.$$eval('.journal .media img', as => as.map(a => a.src.replace("=h180", "=h500")))
    
    // скачиваем обложки
    if(images.length) {
        for (const pic of images) {
            if (!fs.existsSync(Path.resolve(__dirname, 'journals_covers', pic.replace("https://ufa-tr.gazprom.ru/_ah/img/", "") + '.jpg'))) {
                https.get(pic, res => {
                    const f = fs.createWriteStream(Path.resolve(__dirname, 'journals_covers', pic.replace("https://ufa-tr.gazprom.ru/_ah/img/", "") + '.jpg'))
    ...
    const journals = await page.evaluate(() => Array.from(document.querySelectorAll('.journal'), (a) => ({
        ...
        cover: a.querySelector("img").src.replace("=h180", "=h500").replace("https://ufa-tr.gazprom.ru/_ah/img/", "") + '.jpg',
